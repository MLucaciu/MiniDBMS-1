...
public class IndexFilesBerkley {
    ...
    public static void main(String[] args)
        ...
        if (args.length != 2 || !"-h".equals(args[0])) {
            System.err.println
                    ("Usage: java " + IndexFilesBerkley.class.getName() +
                            " -h <envHome>");
            System.exit(2);
        }
        IndexFilesBerkley example = new IndexFilesBerkley(new File(args[1]));
        example.run();
        example.close();
    }
    ...
    private IndexFilesBerkley(File envHome)
            throws DatabaseException {
